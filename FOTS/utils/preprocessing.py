...
def rescale(img, bboxes, scale_x, scale_y):
    ...
    Args:
        img (ndarray / Tensor): (H, W, 3) if ndarray. (3, H, W) if Tensor.
    ...
    if isinstance(img, np.ndarray):
        ...
        new_height, new_width = int(scale_y * height), int(scale_x * width)
        img = transforms.functional.resize(img, size=(new_height, new_width))
...
def random_crop(img, bboxes, texts, crop_size=640, max_attempts=50):
    ...
    if height < crop_size or width < crop_size:
        raise RuntimeError("Image size is smaller than crop size")
...
def pad_to_fixed_size(img, new_height, new_width):
    """ Pad the input image to a predefined width and height. The input image 
    will be at the top-left corner of the output image.

    Args:
        img (ndarray): (old_height, old_width, 3). Input image.
        new_height (int): Height after padding.
        new_width (int): Width after padding.
    
    Returns:
        padded_img (ndarray): (new_height, new_width, 3). Padded image.
    """
    old_height, old_width, _ = img.shape
    if isinstance(img, np.ndarray):
        padded_img = np.zeros((new_height, new_width, 3))
    elif isinstance(img, torch.Tensor):
        padded_img = torch.zeros((new_height, new_width, 3))
    padded_img[:old_height,:old_width,:] = img
    return padded_img

def rescale_rotate_crop(img, bboxes, texts, crop_size=640, max_attempts=50):
    height, width, _ = img.shape
    ...
    # random cropping
    img, bboxes, texts = random_crop(img, bboxes, texts, crop_size=crop_size, max_attempts=max_attempts)
    return img, bboxes, texts

def rescale_with_padding(img, bboxes, texts, size=640):
    height, width, _ = img.shape
    # pad to the longer side
    new_length = max(size, height, width)
    img = pad_to_fixed_size(img, new_length, new_length)
    # rescale to size x size
    scale = size / new_length
    img, bboxes = rescale(img, bboxes, scale_x=scale, scale_y=scale)
    return img, bboxes, texts

def img_aug(img, bboxes, texts, crop_size=640, max_attempts=50):
    ...
    try:
        ...
        if height < crop_size or width < crop_size:
            raise RuntimeError("Image size is smaller than crop size")
        img, bboxes, texts = rescale_rotate_crop(img, bboxes, texts, crop_size, max_attempts)
    except:
        img, bboxes, texts = rescale_with_padding(img, bboxes, texts, crop_size)
    return img, bboxes, texts
...
def generate_rbox(img, bboxes, texts):
    ...
    training_mask = np.ones((height, width))
    for bbox, text in zip(bboxes, texts):
        ...
        cv2.fillPoly(training_mask, [bbox], 0)
        cv2.fillPoly(training_mask, [shrunk_bbox], 1)

        # ignore if the text is 'Don't care'
        if text in ['*', '#', '###']:
            cv2.fillPoly(training_mask, [bbox], 0)
        
        # generate a rotated rectangle that covers the region with minimal area
        min_bounding_rect = cv2.minAreaRect(bbox)
        min_bounding_rect = cv2.boxPoints(min_bounding_rect)
        min_bounding_rect = sort_points_clockwise(min_bounding_rect)

        # find the rotation angle
        angle = find_rotation_angle(min_bounding_rect)
        
        # ignore if the bounding box is too small
        tl, tr, br, bl = min_bounding_rect
        len1 = np.linalg.norm(tl - tr)
        len2 = np.linalg.norm(tr - br)
        if len1 < 10 or len2 < 10:
            cv2.fillPoly(training_mask, [min_bounding_rect], 0)
        
        # for each point inside the shrunk bounding box, compute its distance to
        # the top, right, bottom and right sides of the rectangle
    ...
    score_map = score_map[np.newaxis,::4,::4]
    geo_map = geo_map[:,::4,::4]
    angle_map = angle_map[np.newaxis,::4,::4]
...
def collate_fn(batch):
    ...
    # the img_idx here is the index in the batch, different from the one in data_loaders/datasets.py
    img_idx_mapping = [i for i, bboxes_i in enumerate(bboxes) for _ in bboxes_i]
    imgs = torch.stack(imgs).float()
    score_maps = torch.as_tensor(score_maps, dtype=torch.float)
    geo_maps = torch.as_tensor(geo_maps, dtype=torch.float)
    angle_maps = torch.as_tensor(angle_maps, dtype=torch.float)
    training_masks = torch.as_tensor(training_masks, dtype=torch.float)
